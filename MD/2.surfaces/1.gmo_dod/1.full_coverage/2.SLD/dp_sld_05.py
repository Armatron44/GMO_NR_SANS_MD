...
for typ in types:
    h_df[typ] = (h_df[typ][:half_length] +
                 h_df[typ][half_length:].iloc[::-1].reset_index(drop=True)) / 2
...
h_df.to_csv(f"{name}_sld.csv",
            columns=['z', 'gmo', 'dodecane', 'total'],
            index=False
            )
